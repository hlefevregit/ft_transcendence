...
export const	AIMovePaddle = (pong: React.RefObject<game.pongStruct>): void =>
{
	...
	// Ball is going to the player paddle
	if (previousBallDirection.x > 0)
	{
		// Re-center the AI paddle in the arena
		if (pong.current.paddle1.position.z > 0.2)
			pong.current.paddle1.position.z = game.movePaddleUp(pong, pong.current.paddle1.position.z);
		else if (pong.current.paddle1.position.z < -0.2)
			pong.current.paddle1.position.z = game.movePaddleDown(pong, pong.current.paddle1.position.z);
	}
	// Ball is going to the AI paddle
	else
	{
		// Move the AI paddle to the predicted position of the ball
		lastZ = predictBallImpactZ(pong);
		if (pong.current.paddle1.position.z > lastZ + 0.2)
			pong.current.paddle1.position.z = game.movePaddleUp(pong, pong.current.paddle1.position.z);
		else if (pong.current.paddle1.position.z < lastZ - 0.2)
			pong.current.paddle1.position.z = game.movePaddleDown(pong, pong.current.paddle1.position.z);
	}
}
