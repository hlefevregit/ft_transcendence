...
): baby.StackPanel =>
{
	...
	const	button = baby.Button.CreateSimpleButton(buttonName, buttonText);
			button.width = "200px";
			button.height = "100px";
			button.color = bj.colorsScheme.light1;
			button.background = bj.colorsScheme.dark1;
			button.fontSize = 24;
			button.thickness = 0;
			...
			{
				button.color = bj.colorsScheme.auroraAccent1;
				button.background = bj.colorsScheme.light3;
			});
			button.onPointerOutObservable.add(() =>
			{
				button.color = bj.colorsScheme.light3;
				button.background = bj.colorsScheme.dark1;
			});

	bj.setPaddings(button, "10px");
...
): baby.StackPanel =>
{
	...
	slider.color = bj.colorsScheme.light1;
	slider.background = bj.colorsScheme.dark1;
	slider.thumbColor = bj.colorsScheme.auroraAccent1;
	slider.borderColor = bj.colorsScheme.auroraAccent1;
...
): baby.StackPanel =>
{
	...
	const	title = new baby.TextBlock(titleName, titleText);
			title.width = "50px";
			title.height = "25px";
			title.color = bj.colorsScheme.light1;
...
): baby.StackPanel =>
{
	...
	text.color = bj.colorsScheme.light1;
...
): baby.Container =>
{
	width = width ?? "100%";
	height = height ?? "100%";
	BackgroundColor = BackgroundColor ?? bj.colorsScheme.dark2;
	...
	bj.setAlignment(container, alignment);
...
const	createDummyBlock = (): baby.StackPanel =>
{
...
(
	textName: string,
	labelKey?: bj.labelKey
): baby.StackPanel =>
{
	const	block = createDummyBlock();
	
	// Use the provided labelKey if available, otherwise use textName
	const	keyToUse = labelKey || textName as bj.labelKey;
...
): baby.StackPanel =>
{
	...
	const	keyToUse = labelKey || titleName as bj.labelKey;
...
): baby.StackPanel =>
{
	...
	const	keyToUse = labelKey || buttonName as bj.labelKey;
...
): baby.StackPanel =>
{
	...
	const	cardBackground = new baby.Rectangle(cardName + "Background");
			...
			cardBackground.background = bj.colorsScheme.dark3;
...
): baby.Container =>
{
	...
	const	color: string = bj.colorsScheme.light1;
...
): baby.StackPanel =>
{
	...
	const	background = new baby.Rectangle(inputName + "Background");
			background.width = "100%";
			background.height = "100%";
			background.background = bj.colorsScheme.dark1;
			background.thickness = 0;
	...
	const	input = new baby.InputText(inputName);
			...
			input.width = "100%";
			input.height = "100%";
			input.color = bj.colorsScheme.light1;
			...
			{
				background.background = bj.colorsScheme.dark3;
			});
