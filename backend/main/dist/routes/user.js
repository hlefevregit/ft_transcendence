...
async function setupUserRoutes(fastify) {
    ...
    fastify.put('/api/user/me', { preValidation: [fastify.authenticate] }, async (req, reply) => {
        const userId = req.user.id;
        const { pseudo, avatarUrl, status } = req.body;
        // 1) pseudo ≤ 16 chars
        ...
        if (avatarUrl) {
            const isDataURI = avatarUrl.startsWith('data:image/jpeg') ||
            ...
            if (!isDataURI && !isURL) {
                return reply
                    .status(400)
                    .send({
                    message: 'Invalid avatar format: only JPG/PNG data-URIs or URLs are allowed.',
                });
            }
        }
    ...
    fastify.delete('/api/user/me', { preValidation: [fastify.authenticate] }, async (req, reply) => {
        const userId = req.user.id;
    ...
    fastify.get('/api/user/history', { preValidation: [fastify.authenticate] }, async (req, reply) => {
        const userId = req.user.id;
        ...
        const matches = await Promise.all(games.map(async (g) => {
